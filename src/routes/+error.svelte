...
<div class="justify-center">
	<center class="mt-24 m-auto">
		...
		<div class=" tracking-widest mt-4">
			<span class="prose text-gray-500 text-6xl block"
				><span>{$page.status.toString().split('').join(' ')}</span></span
			>
			<span class="prose text-gray-500 text-xl"
				>เกิดข้อผิดพลาด! กรุณาตรวจสอบ URL หรือกดปุ่มกลับด้านล่าง</span
			>
		</div>
	</center>
	<center class="mt-6">
		<a href="/" class="prose text-gray-500 text-lg bg-gray-200 p-3 rounded-md hover:shadow-md"
			>กลับหน้าหลัก
		</a>
	</center>
</div>
